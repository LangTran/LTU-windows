...
rem set PruneDiffs=
    ...
    rem If your internet connection is unreliable, 
    rem     you can run LangTran Update in "Persistent mode". Then it will try harder.
    set Persistent=no
    rem If you want to run in Persistent mode, delete the "rem" from the next line:
rem set Persistent=yes

    if x%Persistent%x==xnox goto ToArchiveOrNot 
    rem Persistent mode assumes a 20 minute timeout before rsync will give up. (1200 seconds)
    rem You can change that if you want to. Just change the figure below this line.
    set Tmo=1200
